...
5. Добавьте переменные окружения в настройках сервиса:
   ...
   - `ALLOWED_ORIGINS` - URL вашего фронтенда на Vercel (например: `https://your-app.vercel.app`)
     **ВАЖНО:** Убедитесь, что URL начинается с `https://` и не содержит слеш в конце!

6. После деплоя скопируйте URL вашего сервиса (например: `https://leotype-backend.onrender.com`)

**⚠️ Если получаете ошибку CORS:**
- Проверьте, что `ALLOWED_ORIGINS` содержит точный URL вашего фронтенда (например: `https://leotype.vercel.app`)
- URL должен быть без слеша в конце
- После изменения переменных окружения перезапустите сервис на Render
- Проверьте логи на Render - там будут видны проверяемые origins
